...
def process_data(indata):
    ...
    with torch.no_grad():
        output = model(indata)
        energy = energy + 0.01* (torch.norm(output,dim=(0,2))-energy)
    print('Energy of separated speakers', energy)
    pos=torch.argmax(energy)
    print('Speaker with most energy', pos)
    print(indata.size(), output.size())
    output = torch.reshape(output[:,pos,:],(output.size()[2],1))
#    print(output.size())
    return output
