...
class CumulativeLayerNorm1d(nn.Module):
    def __init__(self, num_features, eps=EPS):
        ...
        self.p_num = 0
        self.p_sum=torch.empty(0,0)
        self.p_squared_sum=torch.empty(0,0)
    ...
    def forward(self, input):
        ...
        if self.p_num > 0:
            cum_sum = cum_sum + self.p_sum
            cum_squared_sum = cum_squared_sum + self.p_squared_sum
            cum_num = cum_num + self.p_num

        self.p_sum = cum_sum[:,-1]
        self.p_squared_sum = cum_squared_sum[:,-1]
        self.p_num = cum_num[-1]
