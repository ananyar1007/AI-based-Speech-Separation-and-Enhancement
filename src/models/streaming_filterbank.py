...
from .streaming_tdcn import StreamingConv1d
EPS = 1e-12
...
class Encoder(nn.Module):
    def __init__(self, in_channels, n_basis, kernel_size=16, stride=8, nonlinear=None):
        super().__init__()
        print("Creating streaming encoder")
        self.kernel_size, self.stride = kernel_size, stride
        self.nonlinear = nonlinear

        self.conv1d = StreamingConv1d(in_channels, n_basis, kernel_size=kernel_size, stride=stride, bias=False)
...
class Decoder(nn.Module):
    def __init__(self, n_basis, out_channels, kernel_size=16, stride=8):
        ...
        self.state=torch.empty(0,self.kernel_size)
        self.state_stride=self.kernel_size//(2*self.stride)

    def forward(self, input):
    #    print('Decoder:Input before stat padding',input.size())
        if self.state.numel() >0:
            input = torch.cat([self.state, input], dim=2)
            output = self.conv_transpose1d(input)
            # Default, the first and last strides can be ignored
            output = output[:,:, self.stride:-self.stride]
        else: 
            output = self.conv_transpose1d(input)
    #        print('First output', output.size())
            output = output[:,:,0:-self.stride]       
        
        self.state = input[:,:,-self.state_stride:] 
